...
内置的 9 个写作技能和"暗渡陈仓"范文清单，是 **LLM 自动分类 + 自动蒸馏** 的产物，没经过系统的人工校验：

- 大方向可靠，但逐篇精度不保证——人工抽样看，大约一半到七成是真·目标风格，其余是"沾边"或误判；
- skill 里的占比、频率等数字是模型自己统计的估算值，别当严谨结论引用；
- 当成"粗筛原料"用没问题，当"金标准"用会踩坑。

想把它炼得更纯？思路是：**先人工标注一两百篇做金标准 → 把判定标准写成带边界案例的规则书 → 强模型逐篇单判 + 多模型投票 → 分歧的进人工队列 → 用金标准报精度/召回，达标后再固化进 skill**。本项目没走完这个流程，欢迎 fork 之后自己炼。

> 📌 本项目已归档，不再更新。代码与语料按现状提供，二次加工随意。
